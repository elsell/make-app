...
assert.equal(process.env.APP_ENV, 'production', 'signed mobile releases require APP_ENV=production');
for (const name of ['EXPO_PUBLIC_API_URL', 'EXPO_PUBLIC_OIDC_ISSUER', 'EXPO_PUBLIC_OIDC_CLIENT_ID']) {
  assert.ok(process.env[name], `${name} is required for a production mobile release`);
}
for (const [name, value] of [['EXPO_PUBLIC_API_URL', process.env.EXPO_PUBLIC_API_URL], ['EXPO_PUBLIC_OIDC_ISSUER', process.env.EXPO_PUBLIC_OIDC_ISSUER]]) {
