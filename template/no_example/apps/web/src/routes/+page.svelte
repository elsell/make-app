  ...
  onMount(async () => {
    try {
      ...
      if (session) {
        ...
        const result = await createApiClient(env.PUBLIC_API_URL ?? 'http://localhost:8080', () => session?.token ?? '').GET('/v1/me', { params: { header: { Authorization: `Bearer ${session.token}` } } });
        if (result.error) {
